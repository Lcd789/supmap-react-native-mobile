    ...
    return (
      <View key={index} style={routeInfoStyles.stepItem}>
        {step.maneuver && (
          <MaterialIcons
            name={
              getManeuverIcon(step.maneuver) as keyof typeof MaterialIcons.glyphMap
            }
            size={24}
            color="#2196F3"
        ...
        <View style={routeInfoStyles.stepTextContainer}>
          <Text style={routeInfoStyles.stepInstruction}>{instruction}</Text>
          <Text style={routeInfoStyles.stepDistance}>{distanceText}</Text>
        </View>
      </View>
    );
  };
